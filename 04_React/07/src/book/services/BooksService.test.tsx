...
const mockFetch = async function mockFetch(
  ...
  if (method === "GET" && new RegExp(`^${url}/books/([0-9])$`).test(url)) {
    const id = +url.split(`${url}/books/`)[1];
    return {
      ok: true,
  ...
  if (method === "PUT" && new RegExp(`^${url}/books/([0-9])$`).test(url)) {
    return {
      ok: true,
  ...
  if (method === "POST" && new RegExp(`^${url}/books`).test(url)) {
